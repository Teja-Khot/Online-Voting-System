...
?>
    <div class="alert alert-danger my-3" role="alert">
        Candidate image is too large, please upload small file (max 2MB).
    </div>
<?php
        ...
        <form method="POST" enctype="multipart/form-data">
            <div class="form-group">
                <select class="form-control" name="election_id" required> 
                    ...
                    <?php 
                        // Fetch elections with candidate count in one query
                        $fetchingElections = mysqli_query($db, "
                            SELECT e.*, COUNT(c.id) AS added_candidates 
                            FROM elections e
                            LEFT JOIN candidate_details c ON e.id = c.election_id
                            GROUP BY e.id
                        ...
                        if($isAnyElectionAdded > 0) {
                            while($row = mysqli_fetch_assoc($fetchingElections)) {
                                ...
                                $added_candidates   = $row['added_candidates'];
    ...
    <div class="col-8">
        ...
        <table class="table">
            ...
            <tbody>
                <?php 
                    ...
                    if($isAnyCandidateAdded > 0) {
                        ...
                        while($row = mysqli_fetch_assoc($fetchingData)) {
                            $election_name   = $row['election_topic'] ?? "Election Not Found";
                ...
                ?>
                        <tr> 
                            <td colspan="5">No candidates have been added yet.</td>
                        </tr>
                <?php
                    }
                ?>
...
if(isset($_POST['addCandidateBtn'])) {
    ...
    $candidate_photo_type = strtolower(pathinfo($_FILES['candidate_photo']['name'], PATHINFO_EXTENSION));
    ...
    $new_filename    = uniqid("cand_", true) . "." . $candidate_photo_type;
    $candidate_photo = $targetted_folder . $new_filename;
    ...
    if($image_size < 2000000) { // 2 MB
        if(in_array($candidate_photo_type, $allowed_types)) {
            if(move_uploaded_file($candidate_photo_tmp_name, $candidate_photo)) {
                mysqli_query($db, "
                    INSERT INTO candidate_details(election_id, candidate_name, candidate_details, candidate_photo, inserted_by, inserted_on) 
